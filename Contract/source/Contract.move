module decentralized_reddit::reddit {
    ...
    public entry fun create_post(
        ...
        // Get the store
        let store = borrow_global_mut<RedditStore>(accountAddress);
    ...
    public fun get_post(accountAddress: address, post_id: u64): (address, String, String, u64) acquires RedditStore {
        let store = borrow_global_mut<RedditStore>(accountAddress);
    ...
    public fun get_all_posts(account: address, accountAddress: address): (vector<address>, vector<String>, vector<String>, vector<u64>) acquires RedditStore {
        let store = borrow_global_mut<RedditStore>(accountAddress);
...
// Get the total number of posts
    #[view]
    public fun get_post_count(accountAddress: address): u64 acquires RedditStore {
        let store = borrow_global_mut<RedditStore>(accountAddress);
        store.post_count
    }
}
